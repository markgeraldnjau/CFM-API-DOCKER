...
{
    use ApiResponse,OperationTrait, TransactionTrait, HasApiTokens;
    ...
    public function index(Request $request)
    {
        try {
            ...
            if($request->type == 'CASH') {
                ...
                ->select('tnx.id','trains.train_number','tnx.customer_name','cfm_classes.class_type', 'tnx.trnx_amount','tnx.created_at', 'tnx.trnx_date', 'tnx.trnx_time','operators.full_name',
                'start.station_name as start_name','tnx.trnx_status','end.station_name as end_name','tnx.trnx_number','tnx.trnx_receipt','special_groups.title','tnx.device_number')
                ...
                ->join('cfm_classes', 'cfm_classes.id', '=', 'tnx.class_id')
                ->join('special_groups', 'special_groups.id', '=', 'tnx.category_id')
                ->select('tnx.id','tnx.trnx_status','trains.train_number','tnx.customer_name','cfm_classes.class_type', 'tnx.trnx_amount','tnx.created_at', 'tnx.trnx_date', 'tnx.trnx_time','operators.full_name',
                'start.station_name as start_name','end.station_name as end_name','tnx.trnx_number','tnx.trnx_receipt','special_groups.title','tnx.device_number')
                ...
                ->join('cfm_classes', 'cfm_classes.id', '=', 'tnx.class_id')
                ->join('special_groups', 'special_groups.id', '=', 'tnx.category_id')
                ->select('tnx.id','trains.train_number','tnx.customer_name','cfm_classes.class_type', 'tnx.trnx_amount','tnx.created_at',
                'tnx.trnx_date', 'tnx.trnx_time','operators.full_name','tnx.trnx_receipt',
                'start.station_name as start_name','end.station_name as end_name','tnx.trnx_number','special_groups.title','tnx.device_number','tnx.acc_number','tnx.card_number')
                ...
                ->join('special_groups', 'special_groups.id', '=', 'tnx.category_id')
                ->select('zone_lists.name as zone','tnx.id','trains.train_number','tnx.customer_name','cfm_classes.class_type', 'tnx.trnx_amount','tnx.created_at',
                'tnx.trnx_date', 'tnx.trnx_time','operators.full_name','tnx.trnx_receipt',
                'start.station_name as start_name','end.station_name as end_name','tnx.trnx_number','special_groups.title','tnx.device_number','tnx.acc_number','tnx.card_number')
                ->where('tnx.zone_id', '!=','0')
                    ->orderBy('system_date', 'desc')
                ...
                $lastFiveTransactions = DB::table('ticket_transactions as tnx')
                ->join('operators', 'operators.id', '=', 'tnx.operator_id')
                ->select('tnx.id','tnx.customer_name', 'tnx.trnx_amount','tnx.created_at',
                'tnx.trnx_date', 'tnx.trnx_time','operators.full_name','tnx.trnx_receipt',
                'tnx.trnx_number','tnx.device_number','tnx.acc_number','tnx.card_number')
                ->whereNotNull('tnx.card_number')
    ...
    public function suspicious(Request $request){
        try {
            ...
            ->join('cfm_classes', 'cfm_classes.id', '=', 'tnx.class_id')
            ->join('special_groups', 'special_groups.id', '=', 'tnx.category_id')
            ->select('tnx.id','trains.train_number','tnx.customer_name','tnx.trnx_receipt','cfm_classes.class_type', 'tnx.trnx_amount','tnx.created_at', 'tnx.trnx_date', 'tnx.trnx_time','operators.full_name',
            'start.station_name as start_name','end.station_name as end_name','tnx.trnx_number','special_groups.title','tnx.device_number')
    ...
    public function traincapacity(Request $request){
        try {
            $operatorId = $this->getUserOperatorId();
            if($request->type == 'CASH') {
                $transactions = $this->getTransactions($operatorId,TRAIN_CASH_PAYMENT);
            ...
            $suspiciousTransactions = DB::table('ticket_transactions as tnx')
                ...
                ->select(
                    ...
                    DB::raw('SUM(tnx.trnx_amount) as total_trnx_amount'),
                    DB::raw('COUNT(tnx.trnx_amount) as total_trnx'),
                    DB::raw('tnx.trnx_date'),
                    'tnx.created_at'
                )
                ->groupBy('trains.id', 'trains.train_number', 'tnx.trnx_date', 'train_layouts.total_seats', 'tnx.created_at')  // Include 'tnx.created_at' in the GROUP BY clause
    ...
    public function cancellation(Request $request){
        ...
        return response()->json(['status' => 'success', 'message' => 'Transaction Cancelled successfully'], 201);
    } catch (\Exception $e) {
        Log::error($e->getMessage());
        $statusCode = $e->getCode() ?? 500;
        $errorMessage = $e->getMessage() ?? SERVER_ERROR;
    ...
    public function getZonesTransactions(Request $request)
    {
        try {
            $operatorId = $this->getUserOperatorId();
